...
func (ipc *IPCache) InjectLabels(ctx context.Context, modifiedPrefixes []netip.Prefix) (remainingPrefixes []netip.Prefix, err error) {
	...
	for i, prefix := range modifiedPrefixes {
		...
		if prefixInfo == nil {
			...
			// If the old entry is tainted, and we are attempting to overwrite it,
			// then we want to keep its old source around in case we eventually
			// stop managing it and need to restore the old source
			...
			case oldID.exclusivelyOwnedByLegacyAPI():
				...
				// tainted entry. Store the old (legacy) source
				newOverwrittenLegacySource = oldID.Source
			case oldID.ownedByLegacyAndMetadataAPI():
				// The entry is tainted, but has already been updated at
				// least once by the metadata API. Retain the legacy source as is.
